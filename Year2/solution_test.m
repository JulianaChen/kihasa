...
%S = sspace_small_newcgrid_linear(params0,G);
...
%                 cu_vector(cu_vector < 0) = 0;
                ...
                %A_wide = linspace(-10,max(S.SS_A),length(unique(S.SS_A))); %A_wide = linspace(S.extmin_A,S.extmax_A,length(unique(S.SS_A))); 
                A_tmp = unique(S.SS_A);
                A_wide = [S.extmin_A,A_tmp(2:9)',S.extmax_A];
                ...
                for k = 1:1:G.n_cons
                    ...
                    if x <= 10
                        ...
                        Vmr_next_TVF(k,x) = lambda1*A_next + lambda2*wh_next + lambda3*K_next;
                        % non-regular job:
                        ...
                        Vmn_next_TVF(k,x) = lambda1*A_next + lambda2*wh_next + lambda3*K_next;
                        % unemployed:
                        ...
                        Vmu_next_TVF(k,x) = lambda1*A_next + lambda2*wh_next + lambda3*K_next;
                        % Sector-Specific Value Functions
                        Vm_r(k) = u_r(k) + G.beta * Vmr_next_TVF(k,x); %Vm_r_next;
                        Vm_n(k) = u_n(k) + G.beta * Vmn_next_TVF(k,x); %Vm_n_next;
                        Vm_u(k) = u_u(k) + G.beta * Vmu_next_TVF(k,x); %Vm_u_next;
...
%% output:
if x <= 10
    reg = [cr_vector', Amr_next', Vmr_next, Vmr_next_linear, Vmr_next_TVF]
    nrg = [cn_vector', Amn_next', Vmn_next, Vmn_next_linear, Vmn_next_TVF]
    ump = [cu_vector', Amu_next', Vmu_next, Vmu_next_linear, Vmu_next_TVF]
else
    reg = [cr_vector', Asr_next', Vsr_next, Vsr_next_linear, Vsr_next_TVF]
    nrg = [cn_vector', Asn_next', Vsn_next, Vsn_next_linear, Vsn_next_TVF]
    ump = [cu_vector', Asu_next', Vsu_next, Vsu_next_linear, Vsu_next_TVF]
end

filename = strcat('compare','_j',num2str(j),'_x',num2str(x),'.xls')
